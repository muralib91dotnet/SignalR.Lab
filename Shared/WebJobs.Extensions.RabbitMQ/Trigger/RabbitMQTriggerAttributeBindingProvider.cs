...
namespace Microsoft.Azure.WebJobs.Extensions.RabbitMQ
{
    internal class RabbitMQTriggerAttributeBindingProvider : ITriggerBindingProvider
    {
        ...
        public Task<ITriggerBinding> TryCreateAsync(TriggerBindingProviderContext context)
        {
            ...
            // if (attribute.QueueSetting != null)
            // {
            //    queueName = attribute.QueueSetting.Name;
            // }
            // else
            {
                queueName = Resolve(attribute.QueueName) ?? attribute.QueueSetting?.Name ?? throw new InvalidOperationException("RabbitMQ queue name is missing");
            }
            ...
            if (attribute.QueueSetting != null)
            {
                service = _provider.GetService(connectionString, hostName, attribute.QueueSetting, userName, password, port, attribute.ExchangeSetting);
            }
            else
            {
                service = _provider.GetService(connectionString, hostName, queueName, userName, password, port, deadLetterExchangeName);
            }
